...
namespace WhackAZombie
{
	...
	/// </summary>
	public class WAZGameController : MonoBehaviour 
	{
        ...
        [Tooltip("The chance for a quick zombie to appear. This overrides the chance for a zombie with a helmet to appear")]
        public float quickChance = 0.05f;

        [Tooltip("The chance for a zombie with a helmet to appear")]
        ...
        [Tooltip("A list of targets ( The zombies that appear and hide in the holes )")]
        public WAZZombie[] zombies;
		...
		void ShowTargets( int targetCount )
		{
            ...
            WAZZombie[] previousTargets = GameObject.FindObjectsOfType<WAZZombie>();

            // Go through each object found, and remove it
            foreach (WAZZombie previousTarget in previousTargets)
            {
			...
			while ( targetCount > 0 && maximumTries > 0 )
			{
				...
				int randomTarget = Mathf.FloorToInt(Random.Range(0, zombies.Length));
                ...
                while (spawnPositions[randomPosition].GetComponentInChildren<WAZZombie>() && positionChangeTries > 0)
                {
                ...
                if (!spawnPositions[randomPosition].GetComponentInChildren<WAZZombie>())
                {
                    // Create the target object at the spawn position
                    Transform newTarget = Instantiate(zombies[randomTarget].transform, spawnPositions[randomPosition].position, Quaternion.identity);
                    ...
                    if (Random.value < quickChance) newTarget.GetComponent<WAZZombie>().ShowQuick(hideDelay);
                    else if (Random.value < helmetChance) newTarget.GetComponent<WAZZombie>().ShowHelmet(hideDelay);
                    else newTarget.GetComponent<WAZZombie>().Show(hideDelay);
		...
		public void HitBonus( Transform hitSource, int bonus )
		{
			...
			if ( bonusEffect )
			{
				...
				newBonusEffect.Find("Text").GetComponent<Text>().text = "+" + (bonus * streak).ToString();
...
#else
    PlayerPrefs.SetInt(Application.loadedLevelName + "HighScore", finalScore);
#endif
				}
